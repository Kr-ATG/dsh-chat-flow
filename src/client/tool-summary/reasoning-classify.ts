...
export type ReasoningIconKey =
  | 'pen' | 'search-question' | 'check' | 'clipboard' | 'scales' | 'note' | 'search' | 'chat'

export interface ReasoningCategory {
  readonly label: string
  /** Semantic key rendered as a monochrome SVG glyph (see CategoryIcon). */
  readonly icon: ReasoningIconKey
}

const CATEGORIES: ReadonlyArray<{ label: string; icon: ReasoningIconKey; patterns: RegExp[] }> = [
  {
    label: '实施编写',
    icon: 'pen',
    patterns: [
...
const FALLBACK: ReasoningCategory = { label: '其他', icon: 'chat' }
