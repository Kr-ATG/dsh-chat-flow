 ...
 * 「N 次工具调用 · M 条消息 · K 个 subagent › / 已思考 ›」（control 在前、
 * 插件行在成员位），因此把 control 键整个接管：同一位置只留一行，文案/DOM 与官方逐字一致，
 * 点击行为一分为二——
 *
...
export const TurnProcessShadowView = memo(function TurnProcessShadowView(props: ChatNodeViewProps<'turn-process'>) {
  ...
  // 文案与官方 TurnProcessNodeView 逐字一致：工具调用 + 消息 / subagent
  // 计数（node.data 就是官方 TurnProcessChatData，同口径）；思考数缀在最后
  // （`N 次工具调用 · M 条消息 · K 个 subagent · 思考 T`，与抽屉页签同口径）。
