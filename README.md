...
| **工具调用聚合** | 官方 turn-process 折叠行直接隐藏、各回合强制展开（内容全部平铺，不再折叠）。成员槽位自有行永远渲染（`N 次工具调用 ›`，文案走官方 chat locale 键，中英文自动切换；有思考的回合行尾缀思考数）；总结卡头部的工具/思考 chip 同为抽屉入口。抽屉内容：工具调用总结卡（次数/进行中/失败/按工具分布/涉及文件点击跳转）+ 完整调用树（展开参数/输出、子调用、耗时、下载/长命令进行中实时进度） |
