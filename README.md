...
- **回合呈现**：思考行（官方原生同款折叠行，实时走秒 + 实时文字滚动）· 工具调用行保持官方原生 ·
  对话流卡片（步骤卡 / 总结卡，回合收口才出现）· 思考 / 工具调用气泡（共享活动抽屉，
...
| 能力 | 说明 |
|---|---|
| **思考行** | 官方原生同款折叠行（`已思考 ›` + 摘要 + 底部发丝线）：思考中实时走秒（`思考中 · 12.3s`）+ 实时文字滚动预览（钉底自动跟随，向上翻阅即停），点击**原地展开**看全文（官方语义，Ctrl+F 搜到折叠内容会自动展开）；整回合的聚合思考视图走总结卡「思考 N」chip 的气泡 |
| **工具调用行** | 完全保留官方原生 turn-process 行与原生 ToolCallTree（不再接管、不再影子行）。本回合的工具材料由总结卡负责登记进共享总线，聚合入口 = 总结卡「工具 N」chip：点开气泡看工具调用总结卡（次数/进行中/失败/按工具分布/涉及文件点击跳转）+ 完整调用树（展开参数/输出、子调用、耗时、下载/长命令进行中实时进度） |
| **对话流卡片** | 总结卡头部 = 「本轮完成」徽章 + 统计 chip（用时 / 步骤 / **工具 N** / **思考 N** / Git 次数，后两枚是可点的气泡入口，悬停有 title 说明）；头部单色分层（标签弱化 + 数值加强 + 状态圆点，仅对勾留一笔语义绿，hover 上浮）；全卡片去底色（1px 超细发丝边条 + 轻阴影，深浅主题各配色） | 回合中间的已完成片段 = 轻量步骤卡（无框）；中断回合头部变琥珀色「已中断」。**卡片只在回合结束后出现**，流式期间一律平铺，流式输出不被卡片吞掉 |
