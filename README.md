...
| **工具调用聚合** | 不接管官方 turn-process 行（官方行保持原生，点击即官方内联展开）。抽屉入口有两处：流式/非紧凑模式走成员槽位自有行（`N 次工具调用 ›`，文案走官方 chat locale 键，中英文自动切换；有思考的回合行尾缀思考数）；紧凑模式 closed 回合走总结卡头部的工具/思考 chip。抽屉内容：工具调用总结卡（次数/进行中/失败/按工具分布/涉及文件点击跳转）+ 完整调用树（展开参数/输出、子调用、耗时、下载/长命令进行中实时进度） |
